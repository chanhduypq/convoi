...
//        $("button[type='button']").css("background-color","initial");
        ...
        if(exist_bill_input_number($.trim($('#bill_number').val()),$('input[name="bill_id"]').val())==true){            
...
//                        $(goods[j]).next().css("background-color", color);
        ...
        tien_tam_ung=$("#sum_socai_payment_method3").val();
        ...
        if(tong_tien<tien_tam_ung){
            $("div.error").html("Giá trị hàng hóa (VND) nhỏ hơn số tiền đã đã thanh toán bên sổ cái. Vui lòng nhập lại.").show();
            $("html, body").animate({scrollTop: 0}, "slow");
            return false;
        }
        
        tien_tam_ung=$("#sum_socai_payment_method4").val();
        ...
        tong_tien=parseInt(temp);
        if(tong_tien<tien_tam_ung){
            $("div.error").html("Chi phí ngân hàng (VND) nhỏ hơn số tiền đã thanh toán bên sổ cái. Vui lòng nhập lại.").show();
            $("html, body").animate({scrollTop: 0}, "slow");
            return false;
        }
        
        tien_tam_ung=$("#sum_socai_payment_method5").val();        
        ...
        tong_tien=parseInt(temp);
        if(tong_tien<tien_tam_ung){
            $("div.error").html("Tiền thuế (VND) nhỏ hơn số tiền đã thanh toán bên sổ cái. Vui lòng kiểm tra lại.").show();
            $("html, body").animate({scrollTop: 0}, "slow");
            return false;
        }
