...
<table class="title-HD1 sort">
    <tbody id="listing_container">
        <tr class="title-HD sort">
            <td class="title-HDli title-HDliw65 short_hand_name__full_name">Khách hàng (<label class="yes_no" id="yes">Đã mua hàng</label> / <label class="yes_no" id="no">chưa mua hàng</label>)</td>
        ...
        <?php $this->renderPartial('//customerfull/data_list',array('items'=>$items));?> 
    </tbody>
    <tfoot>
...
$this->renderPartial('//render_partial/common/sort',array('session_key'=>'customer_list_sort','field_array'=>array(
    ...
    jQuery(function ($){
        ...
        $("body").delegate("td.cursor", "click", function() {
            if($(this).find("a").length==1){//link số hóa đơn/ số tờ khai
                ...
                else if ($(this).find("a").eq(0).hasClass("goods")) {//link đến page hàng hóa đã bán
                    id_str = $(this).prev().prev().attr("id");
                    action_for_form_common="<?php echo $this->createUrl('/goodsfull/index'); ?>";
                }
