<h1>Thống kê <?php echo lcfirst(Yii::app()->params['label_for_supplier']);?> nội địa<?php if($all_time_common=='0') echo ": $start_date_common - $end_date_common";?></h1>
<table class="title-HD1 sort">
    <tbody id="listing_container">
        ...
        <?php $this->renderPartial('//supplierfull/data_list',array('items'=>$items));?> 
    </tbody>
    <tfoot>
...
<?php
$this->renderPartial('//render_partial/common/create_branch',array('type'=>Branch::SUPPLIER));
$this->renderPartial('//render_partial/common/edit_branch',array('type'=>Branch::SUPPLIER));
//$this->renderPartial('//supplierfull/index/sort');
$this->renderPartial('//render_partial/common/sort',array('session_key'=>'supplier_list_sort','field_array'=>array(
...
$this->renderPartial('//render_partial/common/delete_branch',array('from_page'=>'supplierfull'));
    ...
    jQuery(function ($){
        $("body").delegate("td.cursor", "click", function() {
            if($(this).find("a").length==1){//link số hóa đơn/ số tờ khai
                if ($(this).find("a").eq(0).hasClass("invoiceinput")) {//link đến page hóa đơn
                    id_str = $(this).prev().attr("id");
                    action_for_form_common="<?php echo $this->createUrl('/invoiceinputfull/index'); ?>";
                }
                else if ($(this).find("a").eq(0).hasClass("invoicechiphi")) {//link đến page hóa đơn
                    id_str = $(this).prev().attr("id");
                    action_for_form_common="<?php echo $this->createUrl('/invoicechiphifull/index'); ?>";
                }
