...
<script type="text/javascript">
    jQuery(function($) {
                ...
                buttons: {
                    "Có": function() {
                      ...
                      $.ajax({ 
                          async: false,
                          cache: false,
                            url: '<?php echo $this->createUrl('/branch/delete'); ?>/id/'+id+'/from_page/<?php echo $from_page;?>',                
        ...
        $("td.cancel_branch").click(function (){
            id=$(this).parents("tr").eq(0).find("td").eq(0).attr("id");
            delete_branch(id);            
        });
        $("body").delegate("td.cancel_branch", "click", function() {
            id=$(this).parents("tr").eq(0).find("td").eq(0).attr("id");
            delete_branch(id);
        });
