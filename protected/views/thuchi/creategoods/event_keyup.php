            ...
            if ($(this).hasClass("price_has_tax")) {//nếu user đang nhập đơn giá có thuế
                ...
                if ($(this).parent().prev().find("input").length > 0) {//nếu đã có <input type="hidden" class="quantity_left" value="xxx">
                    ...
                    $.ajax({
                        ...
                        cache: false,
                        type: "POST",
                        data: {goods_id: $(quantity_node).parent().prev().find("select").eq(0).val(), quantity: quantity_temp, thuchi_id: $('input[name="bill_id"]').val()},
                        url: '<?php echo $this->createUrl("/ajax/checkerrorquantityonegoods1"); ?>',
