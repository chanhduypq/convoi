...
?>
<h1>Thống kê tiền mặt<?php if($all_time_common=='0') echo ": $start_date_common - $end_date_common";?></h1>
...
<table class="title-HD1 sort">
    <tbody id="listing_container">
        ...
        <?php $this->renderPartial('//thuchi/data_list',array('items'=>$items,'index'=>$index));?> 
    </tbody>
    <tfoot>
<!--        <tr>
            <td colspan="9">
...
<script type="text/javascript">
    ...
    function save_thu_chi() {
        ...
        $.ajax({ 
            ...
            cache: false,
            type: "POST",
            url: '<?php echo $this->createUrl('/thuchi/save'); ?>',
            data: {
                thuchi:thu_chi,
    ...
    jQuery(function ($){
        $("button.cursor").click(function (){
            ...
            $.ajax({ 
                ...
                cache: false,
                type: "POST",
                url: '<?php echo $this->createUrl('/thuchi/ketso'); ?>',
                success: function(data, textStatus, jqXHR) {
                    $("#div_loading_customer").hide();
                    submit_form_common('<?php echo $this->createUrl("/".Yii::app()->controller->id."/index"); ?>','<?php echo $this->createUrl("/ajax/search"); ?>');
                }
...
<?php 
$this->renderPartial('//thuchi/create_update');
$this->renderPartial('//thuchi/thuchi_histoty');
if(FunctionCommon::get_role()==Role::ADMIN){
    $this->renderPartial('//thuchi/edit_init'); 
}
...
$this->renderPartial('//render_partial/common/sort',array('session_key'=>'thuchi_list_sort','field_array'=>array(
                                                                            'created_at',
                                                                            'content',
                                                                            'thu',
                                                                            'chi',
