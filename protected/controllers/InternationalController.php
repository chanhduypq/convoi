...
class InternationalController extends Controller {
    ...
    public function actionMore() {        
         ...
         * 
         */
        $items = InternationalFull::model()->findAll($dbCriteria);        
        ...
        if(count($branch_id_array)>0){
            ...
            $items1=  Yii::app()->db->createCommand("select branch_id1,sum(bill_count) as sum_bill_count,sum(tong_tien) as sum_tong_tien,sum(quantity) as sum_quantity from international_full_view where $where and branch_id1 IN ($branch_id_array) group by branch_id1")->queryAll();
        }
        else{
            $items1=array();
        }
...
//        }
    ...
    public function actionIndex() {        
        ...
        //
        $items = InternationalFull::model()->findAll($where1);
        $branch_id_array=array();
        foreach ($items as $value) {
            $branch_id_array[]=$value->branch_id1;
        }
...
//        }
         ...
         */
        $rows=InternationalFull::model()->disableLimitDefaultScope()->findAll($where1);
