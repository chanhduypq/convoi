...
class InvoicefullController extends Controller {
    ...
    public function actionIndex() {
        ...
        if ($this->goods_id_common != "") {  
            ...
            $where.=" and id IN (select bill_id from bill_detail where goods_id=$goods_id_common)";
        }
        $items = InvoiceFull::model()->findAll($where);
        $params['items'] = $items;        
        /**
    ...
    public function actionMore() {        
        ...
        //        
        $items = InvoiceFull::model()->findAll($dbCriteria);
    ...
    public function actionCreate() {
        ...
        if (Yii::app()->request->isPostRequest) {
            $this->create();            
            $this->redirect(array("/invoicefull/index"));
        }
        //
        $model=new Bill();
    ...
    protected function update() {
         ...
         * lưu hóa đơn
         */
        $bill_model = Bill::model()->findByPk(Yii::app()->request->getParam("id"));
         ...
         */
        foreach (BillDetail::model()->findAll("bill_id=" . $bill_model->id) as $bill_detail_model) {
         ...
         */
        if($different==true){
            Common::update_complete_and_socai($bill_model, "thu", "bill_id", "Hóa đơn thương mại");
        }        
        /**
         * 
         */
        $sum=Yii::app()->db->createCommand()
                ->select("sum(thu)")
                ->from("socai")
                ->where("bill_id=" . $bill_model->id . " and thu<>0")
                ->queryScalar();
        if($sum==FALSE||$sum==''){
            $sum=0;
        }
        ...
        if($so_tien_con_lai==0){
            Yii::app()->db->createCommand("delete from socai where bill_id=" . $bill_model->id." and thu=0")->execute();
        }
        else{
            Yii::app()->db->createCommand("update socai set tm=$so_tien_con_lai where bill_id=" . $bill_model->id." and thu=0")->execute();
        }
    ...
    public function actionUpdateandprint1() {
        if (Yii::app()->request->isPostRequest) {            
            ...
            $this->save_history_for_print($bill_id,  BillHistory::PRINT_LIEN1);

            Yii::app()->db->createCommand("update bill set is_printed=1 where id=$bill_id")->execute();
            Yii::app()->runController('invoicefull/lien/id/' . $bill_id.'/print/'.Yii::app()->request->getParam("print")."/lien/1");
            exit;
        }
    }
    ...
    public function actionUpdateandprint2() {
        if (Yii::app()->request->isPostRequest) {
            $this->update();
            $bill_id = Yii::app()->request->getParam("id");            
            $this->save_history_for_print($bill_id,  BillHistory::PRINT_LIEN2);
                  
            Yii::app()->db->createCommand("update bill set is_printed=1 where id=$bill_id")->execute();
            Yii::app()->runController('invoicefull/lien/id/' . $bill_id.'/print/'.Yii::app()->request->getParam("print")."/lien/2");
            exit;
        }
    }
    ...
    public function actionUpdate() {  
        ...
        //
        $params['invoicefull_model'] = InvoiceFull::model()->findByAttributes(array('id'=>$id));
        /**
         * 
         */         
        $params['bill_details']=  BillDetailFull::model()->findAllByAttributes(array('bill_id'=>Yii::app()->request->getParam("id")));
        $params['count_print_lien1_histoty_date'] = BillHistory::getPrintCount($id, BillHistory::PRINT_LIEN1);
        $params['count_print_lien2_histoty_date'] = BillHistory::getPrintCount($id, BillHistory::PRINT_LIEN2);
        ...
        $histoty_array = BillHistory::getBillHistoty($id);
        ...
        $created_user = Yii::app()->db->createCommand()
                ->select("user.danh_xung,user.full_name,date_format(bill.created_at,'$DATE_FORMAT - %H:%i:%s') AS created_at_date")
                ->from("bill")
                ->leftJoin("user", "user.id=bill.user_id")
                ->where("bill.id=$id")
                ->queryRow()
        ;
    ...
    public function actionLien() {        
        $params = array();        
        $bill_details= BillDetailFull::model()->findAllByAttributes(array('bill_id'=>Yii::app()->request->getParam("id")));
        foreach ($bill_details as &$bill_detail) {           
            $model=GoodsNameInitPrintBill::model()->findByAttributes(array('goods_id'=>$bill_detail->goods_id,'bill_id'=>Yii::app()->request->getParam("id")));
            if($model!=NULL){
                $bill_detail->goods_full_name=$model->goods_name;
            }
         ...
         * 
         */        
        $params['invoicefull_model'] = InvoiceFull::model()->findByAttributes(array('id'=>Yii::app()->request->getParam("id")));
    ...
    protected function save_history_for_print($bill_id,$lien) {
        ...
        $bill_detail = Yii::app()->db->createCommand()
                ->select("goods_id,quantity,price,price_has_tax,goods.goods_full_name")
                ->from("bill_detail")
                ->join("goods", "goods.id=bill_detail.goods_id")
                ->where("bill_id=$bill_id")
                ->queryAll()
        ;
        for($i=0;$i<count($bill_detail);$i++){           
            $model=new GoodsNameInitPrintBill();
            $model->setIsNewRecord(true);
            $model->goods_id=$bill_detail[$i]['goods_id'];

            $model->bill_id=$bill_id;
            if($model->findByAttributes(array('goods_id'=>$bill_detail[$i]['goods_id'],'bill_id'=>$bill_id))==NULL){
                $model->goods_name=$bill_detail[$i]['goods_full_name'];
                $model->save(FALSE);
            }
    ...
    protected function update_socai($socai_id,$bill_id,$bill_number){
        $payment_method_id=Yii::app()->db->createCommand()
                    ...
                    ->where("id=$socai_id")
                    ->queryScalar();
        Yii::app()->db->createCommand("update socai set bill_id=$bill_id where id=$socai_id;update bill set payment_method_id=$payment_method_id,is_paying=1 where id=$bill_id")->execute();
        ...
        if(is_array($rows)&&count($rows)==2){
            ...
            $sum_and_sumtax=Yii::app()->db->createCommand()
                ...
                ->where("id=$bill_id")                    
                ->queryScalar();
            $row_trang_thai2=Yii::app()->db->createCommand("select * from socai where bill_id=$bill_id and thu=0 and chi=0")->queryRow();
            if($sum_and_sumtax==$sum_thu_chi){
                Yii::app()->db->createCommand("delete from socai where thu=0 and chi=0 and bill_id=$bill_id")->execute();
                $trang_thai='<img style="width: 39px;height: 39px;" src="'.Yii::app()->theme->baseUrl.'/images/icon/socai/complete.png"/>';
                Yii::app()->db->createCommand("update socai set trang_thai='$trang_thai',tham_chieu='$bill_number',content='".$row_trang_thai2['content']."' where bill_id=$bill_id")->execute();
                Yii::app()->db->createCommand("update bill set is_complete=1 where id=$bill_id")->execute();
            }
            else{
                ...
                Yii::app()->db->createCommand("update socai set trang_thai='$trang_thai1',content='".$row_trang_thai2['content']."',tham_chieu='".$row_trang_thai2['tham_chieu']."' where bill_id=$bill_id and (thu<>0 or chi<>0)")->execute();
                Yii::app()->db->createCommand("update socai set trang_thai='$trang_thai2',tm=".($sum_and_sumtax-($rows[0]['thu']+$rows[0]['chi']))." where bill_id=$bill_id and thu=0 and chi=0")->execute();


            }
            Yii::app()->db->createCommand("update thuchi set content='".str_replace("'", "\'", $row_trang_thai2['content']) ."',bill_id=$bill_id where socai_id=$socai_id")->execute();
            Yii::app()->db->createCommand("update tai_khoan_acb set content='".str_replace("'", "\'", $row_trang_thai2['content'])."',bill_id=$bill_id where socai_id=$socai_id")->execute();
        }
    }
    ...
    protected function create() {
        ...
        if($success==FALSE){            
            ...
            $this->redirect(array("/invoicefull/create"));            
        }
        else{
