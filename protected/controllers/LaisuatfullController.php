...
class LaisuatfullController extends Controller {
    ...
    public function actionIndex() {        
        ...
        $items = LaiSuatFull::model()->findAll($where);
    ...
    public function actionMore() {        
        ...
        //        
        $items = LaiSuatFull::model()->findAll($dbCriteria);
    ...
    public function actionCreate() {
        ...
        if (Yii::app()->request->isPostRequest) {
            $this->create();               
            $this->redirect(array("/laisuatfull/index"));
        }
        
        /**
         * 
         */
        $stt=Yii::app()->db->createCommand("select max(stt) as max from lai_suat")->queryScalar();
        if($stt==FALSE||$stt==""){
            $stt=0;
        }
        $stt++;
    ...
    protected function update() {
         ...
         * lưu hóa đơn
         */
        $bill_model = LaiSuat::model()->findByPk(Yii::app()->request->getParam("id"));
         ...
         */
        if($different==true){
            Common::update_complete_and_socai($bill_model, "thu", "lai_suat_id", "Lãi suất");
        }
        /**
         * 
         */
        $sum=Yii::app()->db->createCommand()
                ->select("sum(thu)")
                ->from("socai")
                ->where("lai_suat_id=" . $bill_model->id . " and thu<>0")
                ->queryScalar();
        if($sum==FALSE||$sum==''){
            $sum=0;
        }
        ...
        if($so_tien_con_lai==0){
            Yii::app()->db->createCommand("delete from socai where lai_suat_id=" . $bill_model->id." and thu=0")->execute();
        }
        else{
            Yii::app()->db->createCommand("update socai set tm=$so_tien_con_lai where lai_suat_id=" . $bill_model->id." and thu=0")->execute();
        }
    ...
    public function actionUpdate() {        
        ...
        //
        $params['invoicefull_model'] = LaiSuatFull::model()->findByAttributes(array('id'=>$id));        
        /**
         * 
         */        
        $update_histoty_array = LaiSuatHistory::getUpdateHistoty($id);        
        ...
        $created_user = Yii::app()->db->createCommand()
                ->select("user.danh_xung,user.full_name,date_format(lai_suat.created_at,'$DATE_FORMAT - %H:%i:%s') AS created_at_date")
                ->from("lai_suat")
                ->leftJoin("user", "user.id=lai_suat.user_id")
                ->where("lai_suat.id=$id")
                ->queryRow()
        ;
    ...
    protected function update_socai($socai_id,$bill_id,$bill_number,$description){
        $payment_method_id=Yii::app()->db->createCommand()
                    ...
                    ->where("id=$socai_id")
                    ->queryScalar();
        Yii::app()->db->createCommand("update socai set lai_suat_id=$bill_id where id=$socai_id;update lai_suat set payment_method_id=$payment_method_id,is_paying=1 where id=$bill_id")->execute();
        $rows=Yii::app()->db->createCommand()
                ->select()
                ->from("socai")
                ->where("lai_suat_id=$bill_id")
                ->order("id ASC")
                ->queryAll();
        if(is_array($rows)&&count($rows)==2){
            ...
            $sum_and_sumtax=Yii::app()->db->createCommand()
                ...
                ->where("id=$bill_id")
                ->queryScalar();
            $row_trang_thai2=Yii::app()->db->createCommand("select * from socai where lai_suat_id=$bill_id and thu=0 and chi=0")->queryRow();
            if($sum_and_sumtax==$sum_thu_chi){
                Yii::app()->db->createCommand("delete from socai where thu=0 and chi=0 and lai_suat_id=$bill_id")->execute();
                $trang_thai='<img style="width: 39px;height: 39px;" src="'.Yii::app()->theme->baseUrl.'/images/icon/socai/complete.png"/>';
                Yii::app()->db->createCommand("update socai set trang_thai='$trang_thai',tham_chieu='$bill_number',content='".str_replace("'", "\'", $description) ."' where lai_suat_id=$bill_id")->execute();
                Yii::app()->db->createCommand("update lai_suat set is_complete=1 where id=$bill_id")->execute();
            }
            else{
                ...
                Yii::app()->db->createCommand("update socai set trang_thai='$trang_thai1',content='".$row_trang_thai2['content']."',tham_chieu='".$row_trang_thai2['tham_chieu']."' where lai_suat_id=$bill_id and (thu<>0 or chi<>0)")->execute();
                Yii::app()->db->createCommand("update socai set trang_thai='$trang_thai2',tm=".($sum_and_sumtax-($rows[0]['thu']+$rows[0]['chi']))." where lai_suat_id=$bill_id and thu=0 and chi=0")->execute();


            }
            Yii::app()->db->createCommand("update thuchi set content='".str_replace("'", "\'", $row_trang_thai2['content']) ."',lai_suat_id=$bill_id where socai_id=$socai_id")->execute();
            Yii::app()->db->createCommand("update tai_khoan_acb set content='".str_replace("'", "\'", $row_trang_thai2['content'])."',lai_suat_id=$bill_id where socai_id=$socai_id")->execute();
        }
    }
    ...
    protected function create() {
        ...
        if($success==FALSE){
            ...
            $this->redirect(array("/laisuatfull/create"));      
        }
        else{
