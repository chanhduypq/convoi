...
class CustomerkxhdfullController extends Controller {
    ...
    public function actionMore() {      
            ...
            if($this->not_buy=='0'){
                ...
                $where = "("
                    ...
                    $where.=" and branch_id1 not in"
                            . "("
                            . "select branch_id1 from customer_kxhd_full_view where ";
                    $where .= "date(created_at) >= '" . FunctionCommon::convertDateForDB($this->start_date_common) . "'";
                    $where.=" and date(created_at) <= '" . FunctionCommon::convertDateForDB($this->end_date_common) . "'";        
                        $where.=")";                        
                $where.=")";
         ...
         * 
         */
        $items = CustomerKxhdFull::model()->findAll($dbCriteria);        
        ...
        if(count($branch_id_array)>0){
            ...
            $items1=  Yii::app()->db->createCommand("select branch_id1,sum(bill_count) as sum_bill_count,sum(tong_tien) as sum_tong_tien from customer_kxhd_full_view where $where and branch_id1 IN ($branch_id_array) group by branch_id1")->queryAll();
        }
        else{
            $items1=array();
        }
...
//                $where.=" or created_at is null)";
        ...
        //
        $items= CustomerKxhdFull::model()->findAll($where1);
        $branch_id_array=array();
        foreach ($items as $value) {
            $branch_id_array[]=$value->branch_id1;
        }
...
//        }
         ...
         */
        $rows=CustomerKxhdFull::model()->disableLimitDefaultScope()->findAll($where1);
        for($i=0;$i<count($rows);$i++){
            if($rows[$i]->bill_count!='0'){
                $row=Yii::app()->db->createCommand()
                    ->select("sum(bill_count) as sum_bill_count,sum(tong_tien) as sum_tong_tien")
                    ->from('customer_kxhd_full_view')
