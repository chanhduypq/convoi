...
class ThuchiController extends Controller {
    ...
    public function actionCreategoods() { 
        ...
        if (Yii::app()->request->isPostRequest) {
            $this->creategoods();            
            $this->redirect(array("/thuchi/index"));
        }  
        /**
         * 
         */
        $min_date=Yii::app()->db->createCommand("select max(created_at) as max from thuchi where is_init=1 AND MONTH(created_at)=".date("m")." AND YEAR(created_at)=".date("Y"))->queryScalar();
    ...
    protected function creategoods() {
        ...
        if($model->type==ThuChi::CHUYEN_KHOAN){
            $model->chuyen_khoan=1;
            $model->khac=0;
        }
        else if($model->type==ThuChi::OTHER){
...
//        ThuChi::update_records($month, $year);
        ...
        $model=  ThuChi::model()->findByPk($model->id);
    ...
    public function actionUpdategoods() {  
        ...
        //
        $params['invoicefull_model'] = ThuChi::model()->findByAttributes(array('id'=>$id));
        /**
         * 
         */         
    ...
    protected function updategoods() {
        ...
        $model=ThuChi::model()->findByPk(Yii::app()->request->getParam("id"));
        $model->content=Yii::app()->request->getParam("content");
...
//        ThuChi::update_records($month, $year);
    ...
    public function actionSave() { 
        ...
        if($id!=""){
            $model=  ThuChi::model()->findByPk($id);
        }
        else{
            $model=new ThuChi();        
            $model->setIsNewRecord(true);
...
//        ThuChi::update_records($month, $year);
    ...
    public function actionEditinit() { 
         ...
         * update tat ca các record thuchi trong tháng hiện tại
         */
        $model->created_at=Yii::app()->db->createCommand()->select("created_at")->from("thuchi")->where("id=$id")->queryScalar();
        $created_at=  explode(" ", $model->created_at);
        $created_at=$created_at[0];
        $created_at=  explode("-", $created_at);
        $year=$created_at[0];
...
//        ThuChi::update_records($month, $year);
    ...
    public function actionMore() {        
        ...
        //        
        $items = ThuChi::model()->findAll($dbCriteria);
    ...
    public function actionIndex() {
        ...
        Yii::app()->session['tm']= Yii::app()->db->createCommand()->select("tm")->from("thuchi")->where("MONTH(created_at)=$month AND YEAR(created_at)=$year AND is_init=1")->order("created_at DESC")->queryScalar();
         ...
         */
        if($this->all_time_common=='1'){
            $where = "id not in (select max(id) from thuchi where is_init=1)";              
        }
        else{
            $where = "date(created_at) >= '" . FunctionCommon::convertDateForDB($this->start_date_common) . "'";
            $where.=" and date(created_at) <= '" . FunctionCommon::convertDateForDB($this->end_date_common) . "'";
        ...
        $items = ThuChi::model()->findAll($where);
        $params['items'] = $items;  
        if($where == "id not in (select max(id) from thuchi where is_init=1)"){
            $count=Yii::app()->db->createCommand()->select("count(*) as count")->from("thuchi")->queryScalar();
        }
        else{
            $count=Yii::app()->db->createCommand()->select("count(*) as count")->from("thuchi")->where($where)->queryScalar();
        }
        
        $params['count'] = number_format($count, 0, ",", ".");
        $row=Yii::app()->db->createCommand()->select("sum(thu) as sum_thu,sum(chi) as sum_chi")->from("thuchi")->where($where)->queryRow();
