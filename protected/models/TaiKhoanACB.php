 ...
 * 
 */
class TaiKhoanACB extends CActiveRecord {
    ...
    public function defaultScope() {
        return $this->getLimitDefaultScopeDisabled() ?
                array(
            'order' => Yii::app()->session['tai_khoan_acb_list_sort'],
                ) :
                array(
            'order' => Yii::app()->session['tai_khoan_acb_list_sort'],
    ...
    public static function delete_record($id, $month, $year) {
        TaiKhoanACB::model()->deleteByPk($id);
        $tm = $so_du_dau_ky = Yii::app()->db->createCommand()->select("tm")->from("tai_khoan_acb")->where("MONTH(created_at)=$month AND YEAR(created_at)=$year AND is_init=1")->queryScalar();
        $rows = Yii::app()->db->createCommand()->select()->from("tai_khoan_acb")->where("MONTH(created_at)=$month AND YEAR(created_at)=$year AND is_init=0")->order("created_at ASC")->queryAll();
        TaiKhoanACB::model()->deleteAll("MONTH(created_at)=$month AND YEAR(created_at)=$year AND is_init=0");

        for ($i = 0; $i < count($rows); $i++) {
            $model = new TaiKhoanACB();
            $model->setIsNewRecord(true);
            if ($rows[$i]['type'] == TaiKhoanACB::CHUYEN_KHOAN) {
    ...
    public static function ket_so() {
        $count=Yii::app()->db->createCommand("select count(*) from tai_khoan_acb where is_lock=0 AND MONTH(created_at)=".date("m")." AND YEAR(created_at)=".date("Y"))->queryScalar();
        if($count!='0'){
            Yii::app()->db->createCommand("update tai_khoan_acb set is_lock=1 where MONTH(created_at)=".date("m")." AND YEAR(created_at)=".date("Y"))->execute();
            $tm =  Yii::app()->db->createCommand()->select("tm")->from("tai_khoan_acb")->where("MONTH(created_at)=".date("m")." AND YEAR(created_at)=".date("Y"))->order("created_at DESC")->queryScalar();
    ...
    public static function update_records($month, $year) {
        ...
        $tm = $so_du_dau_ky = Yii::app()->db->createCommand()->select("tm")->from("tai_khoan_acb")->where("MONTH(created_at)=$month AND YEAR(created_at)=$year AND is_init=1")->order("created_at DESC")->queryScalar();
        $rows = Yii::app()->db->createCommand()->select()->from("tai_khoan_acb")->where("MONTH(created_at)=$month AND YEAR(created_at)=$year AND is_init=0 AND is_lock=0")->order("created_at ASC")->queryAll();
        TaiKhoanACB::model()->deleteAll("MONTH(created_at)=$month AND YEAR(created_at)=$year AND is_init=0 AND is_lock=0");

        $insert="insert into tai_khoan_acb (id,created_at,content,thu,chi,tm,type,bill_id,bill_input_id,bill_chi_phi_id,is_init,kho_hang,chuyen_khoan,khac,is_lock,sxdv_id,kxhd_id,lai_suat_id,chi_phi_khd_id,ung_tien_id,payment_method_id3,payment_method_id4,payment_method_id5,socai_id)";
        for ($i = 0; $i < count($rows); $i++) {
            $value="(";
            $value.=$rows[$i]['id'].",";
            ...
            else if($rows[$i]['type']==TaiKhoanACB::OTHER){
                $value.="0,";
                $value.="1,";
            }
            else{
    ...
    public static function insert_or_update_init_next_month(){
        ...
        $sum=Yii::app()->db->createCommand()->select("sum(thu)-sum(chi)")->from("tai_khoan_acb")->where("MONTH(created_at)=".date('m')." AND YEAR(created_at)=".date('Y')."")->queryScalar();
        if($sum==FALSE||$sum==""){
            $sum=0;
        }
        
        $tm_init=Yii::app()->db->createCommand()->select("tm")->from("tai_khoan_acb")->where("MONTH(created_at)=".date('m')." AND YEAR(created_at)=".date('Y')." AND is_init=1")->queryScalar();
        if($tm_init==FALSE||$tm_init==""){
            $tm_init=0;
        }
        
        $row = Yii::app()->db->createCommand()->select()->from("tai_khoan_acb")->where("MONTH(created_at)=$month AND YEAR(created_at)=$year AND is_init=1")->queryRow();
        if(is_array($row)&&count($row)>0){
            $model1=  TaiKhoanACB::model()->findByPk($row['id']);                
        }
        else{
            $model1=new TaiKhoanACB();
            $model1->setIsNewRecord(true);
