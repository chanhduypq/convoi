 ...
 * 
 */
class SxdvHistory extends CActiveRecord {
    const PRINT_LIEN1=1;
    const PRINT_LIEN2=2;
    ...
    public static function getPrintCount($bill_id,$type){
        $count_print =  SxdvHistory::model()->count("bill_id=$bill_id "
    ...
    public static function getBillHistoty($bill_id){        
        ...
        $histoty_array = Yii::app()->db->createCommand()
                ->select("user.danh_xung,user.full_name,is_preview,print_type,sxdv_history.id,date_format(sxdv_history.updated_at,'$DATE_FORMAT - %H:%i:%s') AS updated_at_date,date_format(sxdv_history.printed_at,'$DATE_FORMAT - %H:%i:%s') AS printed_at_date,data,reason")
                ->from("sxdv_history")
                ->leftJoin("user", "user.id=sxdv_history.user_id")
