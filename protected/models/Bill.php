...
class Bill extends CActiveRecord {
    ...
    public static function getBillNumberForInsert(){
        $max_bill = Yii::app()->db->createCommand()
                    ->select("max(bill_number) as max")
                    ->from("bill")
                    ->queryScalar();
        $max_sxdv = Yii::app()->db->createCommand()
                    ->select("max(bill_number) as max")
                    ->from("sxdv")
                    ->queryScalar();
        if($max_bill==""){
